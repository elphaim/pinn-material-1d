"""
Training module for Weak-Form Physics-Informed Neural Networks
...
class PINNTrainerWeak:
    """
    Trainer class for Physics-Informed Neural Networks.
    
    Args:
        model: PINN model to train
        data_funcs: Dictionary containing test functions
        data_pts: Dictionary containing all training data points
        int_method: Numerical integration method, choice of
            - deterministic methods: 'Simpson', 'Boole' or 'Gaussian'
            - stochastic methods: 'Monte Carlo'
        int_points: Number of sampling points for numerical integration, method-dependent
    ...
    def __init__(
        self,
        model,
        data_funcs: Dict[str, list],
        data_pts: Dict[str, torch.Tensor],
        int_method: str,
        int_points: int,
        device: str = 'cpu',
        ...
        print(f"  Integration method: {int_method} with {int_points} points")
    ...
    def compute_loss_gradients(self) -> Dict[str, float]:
        ...
        # Residual loss
        residual = self.model.compute_weak_residuals(
            test_funcs=self.data_funcs['test_funcs'], 
            test_doms=self.data_funcs['test_doms'], 
            method=self.int_method, 
            n_points=self.int_points
        )
        ...
        u_bc_pred = self.model.forward(self.data_pts['x_bc'], self.data_pts['t_bc'])
        loss_bc = torch.mean((u_bc_pred - self.data_pts['u_bc']) ** 2)
        ...
        u_ic_pred = self.model.forward(self.data_pts['x_ic'], self.data_pts['t_ic'])
        loss_ic = torch.mean((u_ic_pred - self.data_pts['u_ic']) ** 2)
        ...
        if self.model.inverse and 'x_m' in self.data_pts:
            u_m_pred = self.model.forward(self.data_pts['x_m'], self.data_pts['t_m'])
            loss_m = torch.mean((u_m_pred - self.data_pts['u_m']) ** 2)
    ...
    def train_epoch(self, use_lbfgs: bool = False) -> Tuple[Dict[str, float], Optional[float]]:
        ...
        x_m = self.data_pts.get('x_m', None)
        t_m = self.data_pts.get('t_m', None)
        u_m = self.data_pts.get('u_m', None)
        ...
        if not use_lbfgs:
            ...
            total_loss, losses = self.model.loss_function(
                ...
                method=self.int_method,
                n_points=self.int_points,
                x_bc=self.data_pts['x_bc'],
                t_bc=self.data_pts['t_bc'],
                u_bc=self.data_pts['u_bc'],
                x_ic=self.data_pts['x_ic'],
                t_ic=self.data_pts['t_ic'],
                u_ic=self.data_pts['u_ic'],
                x_m=x_m,
                t_m=t_m,
                u_m=u_m,
